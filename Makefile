...
# Terraform provider metadata — used for pull-docs and schema-version-diff.
# The provider itself runs in no-fork mode (embedded via Go module), so no
# Terraform CLI or provider binary is needed at runtime.
...
REGISTRY_ORGS ?= ghcr.io/HPX-Group
IMAGES = $(PROJECT_NAME)
...
XPKG_REG_ORGS ?= ghcr.io/HPX-Group
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/HPX-Group
XPKGS = $(PROJECT_NAME)
